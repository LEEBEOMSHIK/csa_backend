...
final class LegacyMediaPathGuard {

    private LegacyMediaPathGuard() {
    }
    ...
    static Path requireReadableFile(Path configuredRoot, Path candidate) throws IOException {
        ...
        ensureNoLinkOrReparseExistingComponents(target);
        if (!Files.isRegularFile(target, LinkOption.NOFOLLOW_LINKS)) {
            throw new IOException("Legacy media object is unavailable");
        }
        Path realRoot = root.toRealPath();
        Path realTarget = target.toRealPath();
        if (!realTarget.startsWith(realRoot)) {
            throw new IOException("Legacy media real path escapes configured root");
        }
        return target;
    }
    ...
    static Path requireSafeWriteTarget(Path configuredRoot, Path candidate) throws IOException {
        ...
        if (Files.exists(root, LinkOption.NOFOLLOW_LINKS)) {
            if (!Files.isDirectory(root, LinkOption.NOFOLLOW_LINKS)) {
                throw new IOException("Canonical media root is not a directory");
            }
            Path existing = nearestExisting(target);
            Path realRoot = root.toRealPath();
            Path realExisting = existing.toRealPath();
            if (!realExisting.startsWith(realRoot)) {
                throw new IOException("Canonical media real path escapes configured root");
            }
        }
        return target;
    }

    private static void requireLexicalContainment(Path root, Path target) throws IOException {
        if (!target.startsWith(root)) {
            throw new IOException("Media path escapes configured root");
        }
    }

    private static void ensureNoLinkOrReparseExistingComponents(Path target) throws IOException {
        Path current = target.getRoot();
        if (current == null) {
            throw new IOException("Media path has no filesystem root");
        }
        for (Path segment : current.relativize(target)) {
            ...
            if (Files.exists(current, LinkOption.NOFOLLOW_LINKS) && isLinkOrReparsePoint(current)) {
                throw new IOException("Media path contains a link or reparse point");
            }
        }
    }
    ...
    private static Path nearestExisting(Path target) throws IOException {
        ...
        if (current == null) {
            throw new IOException("Media path has no existing ancestor");
        }
        return current;
    }
